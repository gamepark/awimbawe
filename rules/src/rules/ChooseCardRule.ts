...
export class ChooseCardRule extends PlayerTurnRule {
  getPlayerMoves() {
    ...
    if (!cardsInPlayingArea.length) {
      return availableAnimals.moveItems({
    ...
    const allCards: Animal[] = availableAnimals.getItems().map((item) => item.id as Animal)
    return availableAnimals
      .filter((item) => this.canPlay(item.id as Animal, cardsInPlayingArea.getItem()!.id as Animal, allCards))
      .moveItems({ type: LocationType.PlayArea, player: this.player, x: 1 })
  }

  getPlayableCards() {
    const allPlayerCards = this.material(MaterialType.AnimalCard)
      .location((location) => location.type === LocationType.Hand || location.type === LocationType.PlayerColumns)
      .player(this.player)

    return allPlayerCards.filter((item) => {
      ...
      const hasAnItemOnTop = allPlayerCards
        .getItems()
        .some(
          (other) => other.location.type === item.location.type && other.location.id === item.location.id && (other.location.x ?? 0) > (item.location.x ?? 0)
        )
  ...
  getCardRule(move: MoveItem): RuleId | undefined {
    ...
    const cardsInOpponentColumn = opponentCards.location(LocationType.PlayerColumns).length
