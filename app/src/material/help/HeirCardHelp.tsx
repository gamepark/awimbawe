...
export const HeirCardHelp = (props: MaterialHelpProps) => {
  const { item } = props
  ...
  return (
    <div css={marginBottom}>
      ...
      <p>
        {t(playerId === item.id ? `help.heir.own.me` : 'help.heir.own', {
          player: playerName
        })}
      </p>
      {hasWinARound && <div css={italic}>{t(playerId === item.id ? `help.heir.win-round.me` : 'help.heir.win-round', { player: playerName })}</div>}
      <RoundSummaryHelp />
    </div>
  )
}
